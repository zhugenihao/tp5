...
function reGoodsList(page, next) {
    ...
    ajaxMethods({
        url: 'comdysales_promotion/index/',
        type: 'get',
...
function reGoodsHtml(data) {
    ...
    $.each(data, function (i, v) {
        ...
        var cuxiao = '';
        if (v['cp_type'] == 1) {
            var cp_price = v['goods_price'];
            var cuxiao = v['discount'];
        } else if (v['cp_type'] == 2) {
            var cp_price = v['cp_price'];
            var cuxiao = "优";
        } else {

        }
        var html = `<li class="tejiayouhui-li">
                        <a href="` + tp5_url('goods/goods_details?goods_id=' + v['goods_id'] + '&activity=comdysalesp') + `">
                            <div class="tejiayouhui-img"><img src="` + files_url(v['cp_img']) + `"></div>
                            <div class="tejiayouhui-title">` + v['cp_name'] + `</div>
                            <div class="tejiayouhui-bottom">
                                <span class="tejiayouhui-jiage">￥` + cp_price + `</span>
                                <span class="tejiayouhui-xl">` + v['number_payment'] + `人付款</span>
                            </div>
                            <div class="discount-g">` + cuxiao + `</div>
                        </a>
                    </li>`;
        list.push(html);
    });
