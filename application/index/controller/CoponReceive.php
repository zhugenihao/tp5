...
class CoponReceive extends Common {
    ...
    public function addCoponReceive() {
        if ($this->request->isAjax()) {
            ...
            Db::startTrans();
            try {
                $count = couponReceiveModel::getCount(['m_id' => $mid, 'cop_id' => $get['cop_id']]);
                if ($count) {
                    Tiperror("你已经领取了，快去购买吧！");
                }
                $coupon = couponModel::getInfo(['cop_id' => $get['cop_id']]);
                if ($coupon['cop_num'] < 1) {
                    Tiperror("优惠券已被领完！");
                }
                ...
                if ($result) {
                    couponModel::setDecs(['cop_id' => $get['cop_id']], 'cop_num', 1);
