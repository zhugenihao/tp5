...
class ComdysalesPromotion extends Commons {
    ...
    public static function getList($where = [], $limit = 50) {
        ...
        $list = self::alias('c')->join($join)
                        ->field("c.*,g.goods_name,g.thecover,g.goods_price,g.number_payment")->where($where)->order($order)->paginate($limit, false, $map);
        return $list;
    }
    ...
    public static function getComdypInfoTime($where = [], $hours = '') {
        $time = date('Y-m-d H') . ':00:00';
        $hours2 = !empty($hours) ? $hours : $time;
        $hours3 = !empty($hours2 == $time) ? date("Y-m-d H:i:s") : $hours;
        $where['start_time'] = array('elt', $hours2);
        $where['end_time'] = array('egt', $hours3);
