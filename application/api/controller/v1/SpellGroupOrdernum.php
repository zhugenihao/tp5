...
class SpellGroupOrdernum extends Common {
    ...
    public function getlist() {
        if ($this->request->isGet()) {
            $get = input('get.');
            $field = 's.*,count(s.order_status) as member_num,m.photo,m.avatarUrl,m.nickName,m.member_name,sg.sg_members_num,'
...
//            $where = ['s.goods_id' => $get['goods_id'],'s.order_status'=>TOSG_STATUS]; 
            ...
            $goodsRecommendedList = spellGroupOrdernumModel::getList($where, $field, $get['start'], $get['limit']);
    ...
    public function getsgMemberList() {
        if ($this->request->isGet()) {
            ...
            $first_member = memberModel::getMemberInfo(['id' => $get['first_member_id']], 'id,photo,avatarUrl,nickName')->toArray();
