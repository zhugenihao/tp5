...
class Order extends Common {
    ...
    public function orderList() {
        ...
        if ($this->request->isAjax()) {
            $get = input('get.');
            $where['o.m_id'] = $mid;
//            $where['o.state'] = ['neq', SHUTD_STATUS];
            if ($get['state'] != 'all') {
                $where['o.state'] = $get['state'];
            }
            if ($get['search'] != '') {
                $where['g.goods_name|o.order_no'] = array('like', "%" . $get['search'] . "%");
            }
            if ($get['activity'] != '') {
                $where['o.activity'] = $get['activity'];
            }
            $field = 'o.*,g.thecover,g.goods_name,n.goodscolor_id,gc.color_name,cs.cate_name,sg.sg_members_num';
            $orderList = orderGoodsModel::getList($where, $field, $get['start'], $get['limit']);
